...
const generateReceiptText = (): string => {
  ...
  // Totals
  lines.push(padLeft("Subtotal:", 0) + padLeft(formatCurrency(subtotal), 22));
  lines.push(padLeft("Tip:", 0) + padLeft(formatCurrency(tipTotal), 22));
  lines.push(padLeft("Total:", 0) + padLeft(formatCurrency(total), 22));
  lines.push(""); // spacer
  ...
  if (receipt.splitPayment && receipt.payments?.length) {
    ...
    lines.push(padLeft(receipt.paymentMethod, 0) + padLeft(formatCurrency(amountPaid), 22));
  }
